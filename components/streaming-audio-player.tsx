...
import { AUDIO_QUALITY_LEVELS } from "@/lib/audio/audio-streaming";

/**
 * Audio Chapter interface
 */
...
/**
 * Props for the StreamingAudioPlayer component
 */
...
function getSavedPlaybackPosition(storyId: string): PlaybackPosition | null {
  ...
  try {
    ...
    console.error("[STREAMING_PLAYER] Failed to load playback position:", error);
  }
  return null;
}
...
function savePlaybackPosition(position: PlaybackPosition): void {
  ...
  try {
    ...
    console.error("[STREAMING_PLAYER] Failed to save playback position:", error);
  }
}
 ...
 * 
 * Enhanced audio player with streaming optimizations including:
 * - Chapter preloading
...
export function StreamingAudioPlayer({
  ...
  // Get chapter URLs for streaming
  const chapterUrls = chapters.map(ch => ch.audioUrl);
  
  // Use streaming hook
  const streaming = useAudioStreaming({
    chapterUrls,
    preloadAhead,
    enableRangeRequests: enableStreaming,
    autoPreload: enableStreaming,
    onChapterReady: (index, data) => {
      console.log(`[STREAMING_PLAYER] Chapter ${index} ready (${data.byteLength} bytes)`);
    },
    onError: (error) => {
      console.error("[STREAMING_PLAYER] Streaming error:", error);
  ...
  const chapterMarkers = React.useMemo(() => {
    ...
    const markers: { position: number; index: number; preloaded: boolean }[] = [];
    let accumulated = 0;
    ...
    for (let i = 0; i < chapters.length - 1; i++) {
      ...
      markers.push({
        ...
        index: i + 1,
        preloaded: streaming.isChapterPreloaded(i + 1),
      });
    }
    
    return markers;
  }, [chapters, totalDuration, streaming]);
  ...
  // Trigger preloading when chapter changes
  useEffect(() => {
    if (enableStreaming && chapters.length > 0) {
      // Preload upcoming chapters
      streaming.loadChapter(currentChapterIndex);
    }
  }, [currentChapterIndex, enableStreaming, chapters.length]);
    ...
    <TooltipProvider>
      <Card className={cn("overflow-hidden", className)}>
        ...
        <CardContent className={cn("p-4", compact && "p-3")}>
          {/* Header with language selector and streaming status */}
          <div className="flex items-center justify-between mb-4">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="flex items-center gap-1">
                      {streaming.isPreloading ? (
                        <Download className="h-3 w-3 text-blue-500 animate-pulse" />
                      ) : streaming.preloadedChapters.length > 0 ? (
                  ...
                  <TooltipContent>
                    <div className="text-xs">
                      <p>Preloaded: {streaming.preloadedChapters.length} chapters</p>
                      <p>Loaded: {formatBytes(streaming.stats.totalBytesLoaded)}</p>
                      <p>Cache hits: {streaming.stats.cacheHits}</p>
                    </div>
                  </TooltipContent>
                </Tooltip>
              )}
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Quality selector (when adaptive bitrate is enabled) */}
              {enableStreaming && (
                <Select
                  value={streaming.qualityLevel.bitrate.toString()}
                  onValueChange={(v) => {
                    const level = AUDIO_QUALITY_LEVELS.findIndex(
                      (l) => l.bitrate.toString() === v
                    );
                    if (level >= 0) streaming.setQualityLevel(level);
                  }}
                >
                  ...
                  <SelectContent>
                    {AUDIO_QUALITY_LEVELS.map((level) => (
                      <SelectItem key={level.bitrate} value={level.bitrate.toString()}>
                        {level.label} ({level.bitrate}k)
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
          ...
          <div className="relative mb-4">
            ...
            {/* Chapter markers with preload status */}
            {chapterMarkers.map((marker) => (
              <Tooltip key={marker.index}>
                <TooltipTrigger asChild>
                  <div
                    className={cn(
                      "absolute top-1/2 -translate-y-1/2 w-1 h-3 rounded cursor-pointer transition-colors",
                      marker.preloaded
                ...
                <TooltipContent>
                  <p className="text-xs">
                    Chapter {marker.index + 1}
                    {marker.preloaded && " (preloaded)"}
                  </p>
                </TooltipContent>
              </Tooltip>
            ))}
          ...
          {/* Chapter List with preload status */}
                ...
                {chapters.map((chapter, index) => (
                  ...
                  >
                    <span className="flex items-center gap-2">
                      ...
                      ) : streaming.isChapterPreloaded(index) ? (
                        <div className="w-4 h-4 flex items-center justify-center">
          ...
          {/* Streaming Statistics (debug mode) */}
          {showStats && enableStreaming && (
            <div className="mt-4 border-t pt-4 text-xs text-muted-foreground">
              <div className="grid grid-cols-2 gap-2">
                <div>Total loaded: {formatBytes(streaming.stats.totalBytesLoaded)}</div>
                <div>Requests: {streaming.stats.totalRequests}</div>
                <div>Cache hits: {streaming.stats.cacheHits}</div>
                <div>Cache misses: {streaming.stats.cacheMisses}</div>
                <div>Avg load time: {streaming.stats.averageLoadTime.toFixed(0)}ms</div>
                <div>Quality: {streaming.qualityLevel.label}</div>
              </div>
            </div>
          )}
        </CardContent>
